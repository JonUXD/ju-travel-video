...
**Source of Truth:** `staging/` folder contains original high-resolution images
- Keep all original screenshots here (PNG, JPG, JPEG, WebP)
- Organize by video slug: `staging/{slug}/`

**Optimized Output:** `public/images/stills/` contains generated WebP files
- Automatically generated by the optimization script
- Two versions per image:
  - **Gallery:** 1920×1080 WebP @ 80% quality (for future fullscreen gallery)
  - **Thumbnail:** 384×216 WebP @ 85% quality (for carousel display)
- Images are automatically cropped to 16:9 aspect ratio (center crop)

### Commands

```bash
# Process ALL videos (deletes and regenerates all optimized images)
npm run optimize-images

# Process specific video only
npm run optimize-images iceland-2019

# Legacy: Update JSON files without re-optimizing (rarely needed)
npm run update-stills


### Workflow for Adding New Videos

#### Step 1: Prepare Screenshots
1. Extract screenshots from your video (5-12 images recommended)
2. Save them in `staging/{slug}/` (e.g., `staging/iceland-2019/`)
3. Use any format: PNG, JPG, JPEG, or WebP
4. Name them consistently: `{slug}-stills-01.png`, `{slug}-stills-02.png`, etc.

#### Step 2: Run Optimization
```bash
# Process just the new video
npm run optimize-images {slug}

# Or process all videos if you've updated multiple
npm run optimize-images